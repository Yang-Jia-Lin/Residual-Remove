...
def generate_imagenet100_split(
    ...
    dataset = tv_datasets.ImageFolder(
        root=str(img_root), transform=tv_transforms.ToTensor()
    )
    total = len(dataset)
...
def _load_imagenet100(data_root: Path, train: bool, image_size: int) -> Dataset | None:
...
def _load_imagenet1000(data_root: Path, train: bool, image_size: int) -> Dataset | None:
...
def sample_calibration_subset(
    dataset: Dataset, calib_size: int, seed: int = 42
) -> Dataset:
