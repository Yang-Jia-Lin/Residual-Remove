...
def _measure_one(
    ...
    if device.type == "cuda":
        with torch.no_grad():
            # 1. 先把极占显存的 Stem 跑完
            x_stem = model.forward_to_split(
                images, split_point="stem", mode=mode, removed_blocks=removed_blocks
            )
        ...
        with torch.no_grad():
            # 3. 仅测量核心残差阶段和头部的显存峰值
            _ = model.forward_from_split(
                x_stem, split_point="stem", mode=mode, removed_blocks=removed_blocks
            )
...
def main() -> None:
    ...
    output_path = Path(
        ...
        / f"{current_time}_memory_cost.csv"
    )
    ...
    print(
        f"[Exp1-InferenceCost] 延迟测量：预热 {args.latency_warmup} 次，正式 {args.latency_reps} 次"
    )
    ...
    print(
        f"[Exp1-InferenceCost] 测量 batch：{measure_images.shape}（固定，保证公平）\n"
    )
    ...
    for remove_count in range(1, len(blocks) + 1):
        ...
        saved_mem_mb = (
            full_peak_mb - peak_mb if full_peak_mb >= 0 and peak_mb >= 0 else -1.0
        )
        saved_mem_pct = (
            saved_mem_mb / full_peak_mb * 100
            if full_peak_mb > 0 and saved_mem_mb >= 0
            else -1.0
        )
        ...
        mem_str = (
            "N/A" if peak_mb < 0 else f"{peak_mb:.2f} MB (节省 {saved_mem_mb:.2f} MB)"
        )
        print(
            f"          峰值显存：{mem_str}  "
        ...
        rows.append(
            {
                ...
                "saved_memory_pct": round(saved_mem_pct, 2)
                if saved_mem_pct >= 0
                else -1,
                # 推理延迟
    ...
    if len(rows) > 1:
        ...
        if last["peak_mb"] >= 0:
            print(
                f"  峰值显存：{rows[0]['peak_mb']:.2f} MB → {last['peak_mb']:.2f} MB"
